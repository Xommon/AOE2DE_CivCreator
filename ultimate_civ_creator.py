...
def update_civilisation_dropdown():
    ...
    for i, civ in enumerate(civilisation_objects):
        if (civ.name == selected_value):
            ...
            for i, graphic in enumerate([3068, 3076, 3093, 3097, 3072, 3080, 3098, 3075, 3077, 3078, 3094, 3096, 3095, 6322, 6631, 6324, 6323, 7249, 3069, 12499, 3070, 7382, 7648, 8396, 8398, 8400, 8402, 10278, 10280, 10276, 10282, 10473, 10475, 10477, 10479, 6079, 6086, 12396, 12403, 7651, 12485, 12492, 7614, 12602, 12609]):
                if DATA.civs[CURRENT_CIV_INDEX + 1].units[276].standing_graphic[0] == graphic:
                    MAIN_WINDOW.architecture_dropdown.setCurrentIndex(i)
                    break
...
def update_architecture_dropdown():
    ...
    units = DATA.civs[CURRENT_CIV_INDEX + 1].units
    ...
    # Set architecture
    all_units = DATA.civs[CURRENT_CIV_INDEX + 1].units
    for i, current_unit in enumerate(all_units_to_change):
        # Set standing graphic
        try:
            standing_graphic_list = list(all_units[current_unit].standing_graphic)  # Convert to list for modification
            standing_graphic_list[0] = architecture_sets[selected_architecture][i][0]
            all_units[current_unit].standing_graphic = tuple(standing_graphic_list)  # Convert back to tuple
        except Exception as e:
            print('standing:', e)
            pass
        ...
        # Set dying graphic
        try:
            dying_graphic_list = list(all_units[current_unit].dying_graphic)  # Convert to list for modification
            dying_graphic_list[0] = architecture_sets[selected_architecture][i][1]
            all_units[current_unit].dying_graphic = tuple(dying_graphic_list)  # Convert back to tuple
        except Exception as e:
            print('dying:', e)
            pass
        ...
        try:
            for j in range(3):  # Loop through the first 3 damage graphics
                if j < len(all_units[current_unit].damage_graphics):
                    all_units[current_unit].damage_graphics[j].graphic_id = architecture_sets[selected_architecture][i][2 + j]
        except Exception as e:
            print('damaged:', e)
            pass
